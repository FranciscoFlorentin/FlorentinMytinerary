...
const activityController={
       addActivity:(req,res)=>{
        // Instancio una nueva Activity y le paso los parametros mandados a traves desde (req.body)
          ...
          .catch(error=>{ return res.json({sucess:false, error:"Fail to load new Activity"})})
       },
       ...
       getActivitiesByIdItinerary:(req,res)=>{
          Activity.find(req.params).populate("idItinerary")
          .then(data=>{return res.json({sucess:true, response: data})})
          .catch(error=>{return res.json({sucess:false, response: "Fail to get activities by idItinerary"})})
       }
}
